 ...
 * @param {object} noteRulesData - Note rules database for validation
 * @returns {string} Mapped nation code for note rules lookup
 */
function getNationKey(nationCode, module, noteRulesData = null) {
  if (module === 'BA') {
    // Baltic Approaches module mappings
    if (['UK', 'FRG', 'DK', 'NE'].includes(nationCode)) {
      return 'NATO_BA';
    } else if (nationCode === 'US') {
      // For BA module, US naval aircraft use USN/USMC notes
      return 'USN/USMC';
  ...
  if (notes.includes('E') && isAirToGroundCapable) {
    ...
    if (modifiedData.rolledOrdnance) {
      modifiedData.rolledOrdnance = modifiedData.rolledOrdnance
        ...
        .replace(/\s*\+\s*EOGB/g, '')
        .trim();
      // If nothing left after removal, clear it
      if (modifiedData.rolledOrdnance === '') {
...
function applyNATOBA_Notes(modifiedData, notes, tasking, sourceTable, isAirToGroundCapable) {
  ...
  // NATO Note F: No AIM-9 for certain taskings - MODIFICATION
  if (notes.includes('F')) {
  ...
  // NATO Note H: No AIM-9 for Bombing/SEAD - MODIFICATION
  if (notes.includes('H')) {
    if (tasking === 'Bombing' || tasking === 'SEAD') {
      modifiedData.irm = null;
    }
...
function applyPOL_Notes(modifiedData, notes, tasking, sourceTable, isAirToGroundCapable, weaponsData) {
  ...
  // POL Note C: Dual bomb loads for mission range - MODIFICATION
  // Uses higher load for normal missions (Table I), lower load for Deep Strike (Table J/K)
  if (notes.includes('C') && isAirToGroundCapable) {
    applyDualBombLoad(modifiedData, sourceTable, tasking);
  }
  ...
  // POL Note E: Ordnance restrictions - MODIFICATION
  if (notes.includes('E') && isAirToGroundCapable) {
...
function applyDualBombLoad(modifiedData, sourceTable, tasking) {
  if (modifiedData.bomb && modifiedData.bomb.includes('/')) {
    ...
    const isDeepStrike = (sourceTable === 'J' || sourceTable === 'J2' || sourceTable === 'K' || sourceTable === 'K2') 
                         && (tasking === 'Bombing' || tasking === 'SEAD');
    if (isDeepStrike) {
      // Deep Strike: use lower bomb load (longer range, second value)
      modifiedData.bomb = bombParts[1].trim();
    } else {
      // All other missions (including Table I, I2): use higher bomb load (shorter range, first value)
